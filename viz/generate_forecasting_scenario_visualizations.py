    ...
    def generate_scenario_visualization(scenario_path: Path) -> None:
        ...
        for track in scenario.tracks:
            if track.track_id == scenario.focal_track_id :
                print("ego", track.track_id )
            else:
                print("not ego", track.track_id )
        del scenario.tracks[0:3]
        del scenario.tracks[1:-1]
        for track in scenario.tracks:
            if track.track_id == scenario.focal_track_id :
                print("ego", track.track_id )
            else:
                print("not ego", track.track_id )
        # print("*************")
        # print("scenario id: ",scenario.scenario_id, " timestamps: ",scenario.timestamps_ns)
        #print("tracks: ", scenario.tracks)
        parquet_save_path = viz_output_dir / f"{scenario_id}.parquet"
        scenario_serialization.serialize_argoverse_scenario_parquet(parquet_save_path,scenario)
        # print("focal_track_it :",scenario.focal_track_id)
        # print("map id : ",scenario.map_id , " slice id: ",scenario.slice_id)
        # print("*************")
